...
fn main() {
    ...
    loop {
        ...
        let mut mutator = lib::Mutator::new(input, 0x2839839283234 ^ unsafe { std::arch::x86_64::_rdtsc() });
        ...
        if exit.signal()  == Some(11){
            println!("Iteration: {}", iter);
            println!("Bug");
            std::fs::write(format!("./output/crash_{}_{}", iter, filename), mutator.input).unwrap();
            break;
        }
        iter += 1;
    }
